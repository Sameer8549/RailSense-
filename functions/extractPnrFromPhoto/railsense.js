...
async function extractPnrWithNvidia(imageBase64) {
      ...
      messages: [
        {
          role: "user",
          content: [
            { type: "text", text: "Read this Indian Railways ticket. Return only JSON with pnr (exactly 10 digits or null), trainNumber (five digits or null), coach (such as B2 or S4 or null), berth (seat or berth number or null), and confidence (0 to 1). Do not invent values." },
  ...
  try {
    ...
    // Some NIM model versions return plain text despite response_format.
  }
  const pnrMatch = String(parsed.pnr || content).match(/\b(\d{10})\b/);
  const trainMatch = String(parsed.trainNumber || content).match(/\b(\d{5})\b/);
  const coachMatch = String(parsed.coach || content).match(/\b([A-Z]{1,2}\d{1,2})\b/i);
  const confidence = Math.max(0, Math.min(1, Number(parsed.confidence || 0)));
  return {
    pnr: pnrMatch ? pnrMatch[1] : null,
    trainNumber: trainMatch ? trainMatch[1] : null,
    coach: coachMatch ? coachMatch[1].toUpperCase() : null,
    berth: parsed.berth ? String(parsed.berth).replace(/[^A-Za-z0-9 -]/g, "").slice(0, 12) : null,
    confidence,
    provider: "nvidia_nim",
